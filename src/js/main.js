...
let navbarText = {
  ...
  productScanner: { header: 'Product Scanner', paragraph: 'Search packaged foods by name or barcode' },
  foodLog: { header: 'Food Log', paragraph: 'Track your daily nutrition and food intake' },
  mealDetails: { header: 'Recipe Details', paragraph: 'View full recipe information and nutrition facts' }
}

let nutritionGradeRating = { a: 'Excellent', b: 'Good', c: 'Average', d: 'Poor', e: 'Bad', nknown: 'Unknown' };
let novaGroupState = { 1: 'Unprocessed', 2: 'Processed Ingredients', 3: 'Processed', 4: 'Ultra-processed' };
...
const areaBtns = document.querySelectorAll('#search-filters-section > div > div > button');
...
const IngredientsMealDetails = document.querySelector('#Ingredients-meal-details');
const IngredientsCount = document.querySelector('#Ingredients-count');
const InstructionsMealDetails = document.querySelector('#Instructions-meal-details');
const nutritionFactsMealDetails = document.querySelector('#nutrition-facts-container');
...
window.displayClickedMealDetails = async function (id) {
  ...
  updateNavbarText(navbarText.mealDetails.header, navbarText.mealDetails.paragraph);
  window.scrollTo(0, 0);
...
window.displayClickedProduct = function (barcode) {
  let clickedProduct;
...
function showProductspage() {
  ...
  updateNavbarText(navbarText.productScanner.header, navbarText.productScanner.paragraph);
}
  ...
  for (const element of loggedItemsList) {
    items += `<div class="flex items-center justify-between bg-gray-50 rounded-xl p-4 hover:bg-gray-100 transition-all">
                        <div class="flex items-center gap-4">
                            ${element.itemType == itemType.recipe ? `<img src="${element.src}" alt="${element.name}" class="w-14 h-14 rounded-xl object-cover">`
...
class LoggedItemsClass {
  constructor(name, brandOrservings, itemType, calories, protein, carbs, fat, src) {
...
function createItem(clickedProduct = 0) {
  let item;
  if (clickedProduct) {
    item = new LoggedItemsClass(clickedProduct.name, clickedProduct.brand, itemType.product, Math.ceil(clickedProduct.nutrients.calories), +clickedProduct.nutrients.protein.toFixed(1), +clickedProduct.nutrients.carbs.toFixed(1), +(clickedProduct.nutrients.fat * 0.4).toFixed(1), null)
  }
  else {
    let calPerServing;
    let protein;
...
function disableLogMealBtn() {
  ...
  logMealBtn.classList.add('bg-gray-300', 'text-gray-500', 'cursor-not-allowed')
